...
export function Contact() {
  ...
  const saveUser = (e) => {
    e.preventDefault();
    localStorage.setItem("name", "ashfak");
  };
  return (
    <>
      <div
        id="contact"
        className="bg-[#091c2a] flex flex-col sm:flex-row gap-10 py-10 px-5 justify-center"
      >
        <div className="bg-white p-5 max-w-[350px] flex flex-col gap-3 rounded-md shadow-xl">
          ...
          <form action="#" className="flex flex-col justify-center gap-5">
            ...
            ></textarea>
            <button
              className="bg-[#04dae5] w-[120px] text-white p-2 rounded-3xl text-sm"
              onClick={saveUser}
            >
              Sent Massage
            </button>
